class Rms::ReportTemplateNew < External
  set_sequence_name "wlan_seq_config"
  set_table_name "WLAN_RMS_TEMPLATE_NEW"

  has_many :chart_sets, :class_name => "Rms::ChartSet", :foreign_key => 'template_id', :order => "id"
  has_many :report_template_users, :class_name => "Rms::ReportTemplateUser", :foreign_key => "template_id"
  ...
  def apply_default_value(perf_template)
    ...
    #        Rms::ReportTemplateUser.new({:user_id => user_id})
  ...
  def create_chart_set(charts)
    ...
    charts.each { |chart|
      chart_sets.create(Rms::ReportTemplateSet.adjust_attributes(chart))
    }
  end
  ...
  def build_netloc_for_common_and_cmcc user
    self.netloc = Rms::ReportTemplate.get_netloc(user.domain.base)
  ...
  class << self
    def clear_associations(ids)
      ...
      Rms::ReportTemplateSet.delete_all("TEMPLATE_ID in (#{ids.join(",")})")
      Rms::ReportTemplateUser.delete_all("TEMPLATE_ID in (#{ids.join(",")})")

      puts "del pk "+ids.join(",");
    end
    ...
    def index_type(netgran)
      types = Rms::PerfType.find(:all, :conditions=>get_index_by_netgran(netgran) + " and index_sys = 1", :order=>'order_seq,name_cn')
      types.collect{|u| { :id => u.id, :name => u.name_cn } } if types && types.size > 0
    end
