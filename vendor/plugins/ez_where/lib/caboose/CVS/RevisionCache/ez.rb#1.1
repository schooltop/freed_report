...
module Caboose
  ...
  module EZ
    ...
    module ClassMethods
      ...
      def ez_find_with_joins(what, *args, &block)
        options = args.last.is_a?(Hash) ? args.last : {}
        options[:joins] ||= []
        options[:joins] = [options[:joins]] if options[:joins].kind_of?(Symbol)
        ...
        if block_given?
          ...
          case options[:joins]
          when Hash
            options[:joins].each { |key, value|
              conditions << create_condition_with_assoc(key, outer_mapping, inner_mapping)
              if(value.is_a?(Array))
                value.each { |a|  conditions << create_condition_with_assoc(key, outer_mapping, inner_mapping) }
              end
            }
          when Array
            options[:joins].uniq.each do |assoc|
              case assoc
              when Symbol
                conditions << create_condition_with_assoc(assoc, outer_mapping, inner_mapping)
              when Hash
                assoc.each {|key, value|
                  conditions << create_condition_with_assoc(key, outer_mapping, inner_mapping)
      ...
      def create_condition_with_assoc(assoc,  outer_mapping, inner_mapping)
        ...
        assoc_klass.ez_condition(cond_options)
      end
      ...
      def ez_find(what, *args, &block)
        options = args.last.is_a?(Hash) ? args.last : {}
        options[:joins] ||= []; options[:joins] = [options[:joins]] if options[:joins].kind_of?(Symbol)
        outer_mapping = options.delete(:outer) || {} # preset :outer value for each :include subcondition, defaults to :and
        outer_mapping.default = :and
        inner_mapping = options.delete(:inner) || {} # preset :inner value for each :include subcondition, defaults to :and
        inner_mapping.default = :and
