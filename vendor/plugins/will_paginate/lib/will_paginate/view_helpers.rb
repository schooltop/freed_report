...
module WillPaginate
  ...
  class LinkRenderer2 < WillPaginate::LinkRenderer
    def page_link_or_span(page, span_class = 'current', text = nil)
      text ||= page.to_s
      if page and page != current_page
        @template.link_to text, url_options(page), {:onclick=>"if(!this.href) return false;$(\"#query_condition\").attr(\"action\", this.href).submit();return false;"}
      else
        @template.content_tag :span, text, :class => span_class
      end
    end
